...
export const PlayerSeat: React.FC<PlayerSeatProps> = ({
   ...
   return (
      <React.Fragment>
         {/* Current Bet (Chips) */}
         {player.currentBet > 0 && (
            <div className={`absolute flex flex-col items-center justify-center -translate-x-1/2 -translate-y-1/2 z-10 transition-all ${isFolded ? 'opacity-70' : ''}`} style={{ left: `${50 + (27 + pushFactor * 8) * Math.cos(angle)}%`, top: `${50 + (20 + pushFactor * 8) * Math.sin(angle) + (isHero ? 11 : (isTop ? -10 : 0))}%` }}>
